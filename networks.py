...
import numpy as np
#critic:judge the value of an action when paired with a particular state, judge how good an action is in a state
...
class CriticNetwork(nn.Module):
    def __init__(self,input_dims,n_actions,fc1_dims = 256,fc2_dims = 128,name = 'critic',checkpoint_dir='tmp/td3',learning_rate = 10e-3):
        ...
        #Adam optimizer with updated weight decay capabilities
...
class ActorNetwork(nn.Module):
    def __init__(self,input_dims, fc1_dims=256, fc2_dims=128,learning_rate = 10e-3, n_actions = 2, name = 'actor', checkpoint_dir='tmp/td3'):
        ...
        self.checkpoint_dir = checkpoint_dir
        self.checkpoint_file = os.path.join(self.checkpoint_dir, self.name + '_td3')
        #just taking an action based on state
        self.fc1 = nn.Linear(*self.input_dims,self.fc1_dims)
        self.fc2 = nn.Linear(self.fc1_dims,self.fc2_dims)
        self.output = nn.Linear(self.fc2_dims,self.n_actions)
        #using adam in both placing let to less exploration ans setting adam w in both places, the agent couldn't learn
